...
export class EditVentaComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.updateDate();
      ...
      this.ventaDiamante = this.diamanteSvc.venta;
      // this.imgRef.push(...this.ventaDiamante.image);
  }
        ...
        .pipe(finalize(() =>{
          ref.getDownloadURL().subscribe((url)=>{
           // this.imgRef = this.imgRef.concat([url]);
            this.ventaDiamante.image = this.ventaDiamante.image.concat([url]);
  ...
  crearVenta(){
    ...
    }else if( this.ventaDiamante.image.length < 1){
       swal('Error','Por favor sube una imagen como minimo', 'error');
    }else{
      console.log(this.ventaDiamante);
     // this.ventaDiamante.image.push(...this.imgRef);
      this.diamanteSvc.updateVenta(this.ventaDiamante);
      swal({
        closeOnClickOutside: false,
        title: "Exito",
        text: "Se a actualizado la venta :D",
        icon: "success",
        buttons: ['cancelar','Regresar a ventas'],
        dangerMode: true,
      })
      .then((willDelete) => {
        if (willDelete) {
  ...
  deleteImage(img: string, index: any): void{
    this.diamanteSvc.deleteImageDB(img).subscribe(res => {
      this.ventaDiamante.image.splice(index,1);
    },
    err => {
